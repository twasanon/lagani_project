...
import { X } from 'react-reusables/lib/icons/X';
...
// Define a more specific props type for the Native overlay wrapper
type DialogOverlayNativeProps = Omit<DialogPrimitive.OverlayProps, 'children'> & {
  children?: React.ReactNode; // Ensure children is ReactNode for Animated.View compatibility
};

const DialogOverlayNative = React.forwardRef<
  DialogPrimitive.OverlayRef,
  DialogOverlayNativeProps // Use the refined props type here
>(({ className, children, ...props }, ref) => {
  return (
    <DialogPrimitive.Overlay
      style={StyleSheet.absoluteFill}
      className={cn('flex bg-black/80 justify-center items-center p-2', className)}
      {...props} // Pass remaining props (excluding children) to the primitive
      ref={ref}
    >
      <Animated.View entering={FadeIn.duration(150)} exiting={FadeOut.duration(150)}>
        {/* Now 'children' matches the type expected by Animated.View */}
        {children}
      </Animated.View>
    </DialogPrimitive.Overlay>
  );
...
const DialogOverlay = Platform.select({
  ...
  default: DialogOverlayNative as any,
});
